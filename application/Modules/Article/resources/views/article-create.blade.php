...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">ساخت مقاله</li>
        </ol>
    </nav>
                    ...
                    <form action="{{route('article.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-2 mb-5">
                                ...
                                <select class="form-control @error('status') is-invalid @enderror" name="status">
                                    ...
                                    @foreach(\Modules\Article\Models\Article::STATUS as $status)
                                        ...
                                        @endphp
                                        <option value="{{$status}}">{{$status_fa}}</option>
                                    @endforeach
                                </select>
                                @error('status')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-5">
                                ...
                                <textarea type="text" class="form-control @error('body_fa') is-invalid @enderror" id="exampleInputText1" name="body_fa" rows="8">{{old('body_fa')}}</textarea>
                                @error('body_fa')
                                <span class="invalid-feedback" role="alert">
                                     <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <div class="col-md-6 mb-5">
                                ...
                                <textarea type="text" class="form-control @error('body') is-invalid @enderror" id="exampleInputText1" name="body" rows="8">{{old('body')}}</textarea>
                                @error('body')
                                <span class="invalid-feedback" role="alert">
                                     <strong>{{ $message }}</strong>
                                </span>
